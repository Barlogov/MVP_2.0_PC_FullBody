...
public class ModelCalibrator : MonoBehaviour
{
    ...
    */
    ...
    public string WriteDataToRecordFile()
    {
        string path = "ModelRecordValues.json";
        using FileStream fileStream = File.Open(path, FileMode.OpenOrCreate);
        fileStream.Close();

        StreamReader recordReader = new StreamReader(path);
        string result = recordReader.ReadLine();
        recordReader.Close();

        (float x, float y, float z) SpineBasePosition = jointPositions[(int)Kinect.JointType.SpineBase];
        if (SpineBasePosition.z == 0)
        {
            result = $"You did not appear in the camera frame";
        }
        else
        {
            StreamWriter calibrationWriter = new StreamWriter(path);
            result = JsonConvert.SerializeObject(jointPositions);
            calibrationWriter.WriteLine(result);
            //AssetDatabase.ImportAsset(path);
            calibrationWriter.Close();
        }
